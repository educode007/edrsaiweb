        ...
        while True:
            ...
            while '\n' in buf:
                ...
                try:
                    ...
                    azim  = _to_float(_pick_part(parts, ['AZIM', 'AZM', 'AZIMUTH']))
                    ...
                    if gamma is None and incl is None and azim is None and gtfa is None and mtfa is None and points is None and sender_ts is None:
                        continue
    ...
    while not _serial_stop.is_set():
        try:
            ...
            while '\n' in buf:
                ...
                if key is None:
                    continue
                # Serial should contribute depth only; simulator owns non-depth channels.
                if key not in ('hole_depth', 'bit_depth'):
                    continue
                with _lock:
                    _latest[key] = val
...
def _simulator():
    ...
    while not _sim_stop.is_set():
        ...
        try:
            with _lock:
                gamma_offset  = _config.get('gamma_offset', 15.0)
                serial_active = bool(_config.get('port', ''))
                skew          = _tcp_time_skew or 0.0
                # When serial is active, use real depths from _latest; otherwise use simulated depths
                final_hd = (_latest.get('hole_depth') or hd) if serial_active else hd
                final_bd = (_latest.get('bit_depth')  or bd) if serial_active else bd
                # Record gamma only when depth advances (HD2 > HD1) and a new TCP packet arrived
                hd_advancing = (
                    _last_hd_sample is not None and
...
def api_config_app_set():
    ...
    try:
        ...
        pass
    return jsonify({'ok': True, 'gamma_offset': _config['gamma_offset'], 'tcp_port': _config['tcp_port']})
